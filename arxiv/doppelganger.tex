...
of training, it recovers the exact real source about $80\%$ of the time (chance $0.03\%$), on six
different encoders, whereas recognizing the mere sound event does not carry over. The learned matching is
specific to one generator---it survives changes to that generator's settings but not a switch to a
different generator, and collapses for text-only synthesis. A human annotation baseline ($49$
listeners) lands well above chance but below the models. Synthetic twins fool people into calling them
real about $29\%$ of the time, yet a detector separates them perfectly.
\end{abstract}

\begin{figure}[h]
\centering
\caption{Matching a sound effect to its synthetic twin, and what transfers to unseen sound events.}
\label{fig:teaser}
...
baseline (dotted line), while no training improves retrieval of other clips of the same kind.
...
instance correspondence to unseen sound events (full-gallery R@1 $0.800$, all five folds),
...
\begin{table}[t]
\centering
\caption{Where Doppelganger sits among related benchmarks.}
\label{tab:prior}
...
\label{sec:bench}
\paragraph{Core corpus (DCASE-T7).} The DCASE-2023 Task-7 corpus~\citep{choi2023foley} provides a ready-made, event-aligned dataset of real and synthetic audio clips spanning 7 classes. It comprises 5{,}550 real clips, sourced from FSD50K~\citep{fonseca2022fsd50k}, UrbanSound8K~\citep{salamon2014urbansound}, BBC, and Freesound. For the experiments, I adopt DCASE's source-disjoint dev/eval split as train-val/test. In addition, the corpus includes 25{,}900 synthetic clips generated by 37 different systems. This setup offers a controlled environment for measuring the clean gap between real and synthetic data, as well as for conducting leave-one-class-out probes. Table~\ref{tab:corpus} panel (a) provides an overview. 
...
\begin{table}[t]
\centering
\caption{Frozen encoders carry a genuine synthetic--real gap (both corpora, test split).}
\label{tab:frozen}
...
query, real gallery); synth$\to$real mAP = cross-domain sound-event retrieval (synthetic query, real
gallery); gap = control $-$ cross, the mAP lost by crossing the domain boundary; PAD =
...
\begin{table}[t]
\centering
\caption{The invariant and sensitive heads are mirror images on both corpora (within-taxonomy).}
\label{tab:heads}
...
training). control = same-domain sound-event mAP (real query, real gallery); cross =
...
CIs over queries. Per encoder, a class-supervised and an instance-pair head are trained per fold
with identical capacity and schedule. The instance head beats frozen in every fold of every
...
\begin{table}[t]
\centering
\caption{The dissociation on unseen sound events (five-fold leave-classes-out, CLAP).}
\label{tab:diss}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{lcc}
\toprule
...
against the full real test gallery ($N=3{,}065$ distractors of all sound events, chance $=0.0003$),
with 95\% bootstrap CIs over queries. The instance objective (positive $=$ a clip and its own twin) reaches R@1 0.800, far
...
\label{sec:human}
I collected a human annotation baseline to place the model numbers on a human scale. Fifty annotators on Prolific ($49$ after excluding those who failed both attention checks), each completing $16$ real-vs-twin discrimination trials (selecting the real clip from two) and $24$ twin-retrieval trials plus $2$ catch trials, all using the same clips as the models (annotators heard the central $3.5$\,s of the $5$\,s model window). Table~\ref{tab:human} shows that humans perform between chance and the models on both tasks.
...
\begin{table}[t]
\centering
\caption{People land between chance and the models on both tasks.}
\label{tab:human}
\renewcommand{\arraystretch}{1.25}\small
\begin{tabular}{lccc}
\toprule
...
\bottomrule
\end{tabular}
\fnote{Accuracy (\%) on the identical trials shown to annotators. $N=49$ Prolific annotators
(3 excluded for missing both attention checks), $771$ discrimination and $1{,}129$ retrieval
...
six-choice task, frozen CLAP and the instance head each pick the nearest of the same six candidates.
...
\label{sec:xfer}
Rather than infer the boundary, I measure it directly by generating twin sets under four generation conditions beyond the core corpus: two more Stable-Audio operating points (\texttt{init\_noise} $0.45$ and $0.75$), a second audio-conditioned family (AudioLDM style-transfer at strength $0.5$~\citep{liu2023audioldm}), and a second text-only generator (a whoosh-domain diffusion model, evaluated on whoosh-sound event anchors). I then cross these with instance heads trained on each family, as shown in Table~\ref{tab:xfer}.
...
\paragraph{Text-only generators.} Both text-only generators fail in the same way. ElevenLabs and the Woosh generator achieve frozen R@1 $0.02$, and no head improves on this, because the caption bottleneck results in twins that are event-appropriate but not true renderings of the specific clip. The learned mapping is therefore specific to audio-conditioned generation and to the generator family. It serves as an instance-correspondence inverse for a single family, not as a universal synthetic-to-real transformation.

\begin{table}[t]
\centering
\caption{Cross-generator transfer matrix (instance R@1, full real-test gallery).}
\label{tab:xfer}
\renewcommand{\arraystretch}{1.25}\small
...
\bottomrule
\end{tabular}
\fnote{Synthetic$\to$real exact-twin retrieval against the full real test gallery ($N=3{,}065$,
chance $0.0003$); fidelity = mean CLAP cosine between each twin and its source.
SAO-trained head = the paper's instance heads (trained on Stable-Audio noise-0.6 pairs),
evaluated under the five-fold unseen-event protocol; ALDM-trained head = an instance head
trained on AudioLDM pairs instead. Rows 1--3: the mapping transfers across operating points
of the training family. Row 4: it does not transfer across families, in either
...
generation breaks instance correspondence for every model and no head recovers it (Woosh: 86
whoosh-sound event queries; ElevenLabs: 670 train+val queries against an $N=7{,}355$ gallery, as no
test-split EL twins exist).}
\end{table}


\begin{figure}[t]
\centering
\caption{The dissociation holds across six encoders (unseen sound events, full gallery).}
\label{fig:diss}
...
generalizes across sound events; sound-event structure essentially does not.}
\end{figure}

\begin{figure}[t]
\centering
\caption{The instance head is not just matching near-copies (fidelity sweep).}
\label{fig:fidelity}
...
The answer is a dissociation. Instance correspondence generalizes across sound events while category
structure does not, and it holds within one generator family but not across families. The broader
...
\paragraph{Collection and preprocessing.} No primary collection. The pipeline programmatically
...
twins ship with the full generation log (event, prompt, seed, steps, cfg, noise level), so every
clip is bit-reproducible with Stable Audio Open without redistributing restricted audio.
...
\label{app:human}
Annotators ($N=50$, $49$ analyzed) were recruited on Prolific, desktop with working audio
...
An annotator who missed both catch trials was excluded from analysis (3 of 50). Everyone who
finished was paid regardless. Human 95\% CIs are bootstrapped over annotators. Collection stored
only the Prolific ID, per-trial choices, and response times. Annotators consented on the first
...
\begin{figure}[h]
\centering
\caption{The geometry flips between the invariant and sensitive heads.}
\label{fig:umap}
...
held out of head training, pooled over the five folds; UCS CatIDs sorted by instance-head R@1
(left column high, right column low). $n$ = number of queries per sound event. The instance head
...
\label{fig:appendix}
\begin{subfigure}{0.49\textwidth}\includegraphics[width=\linewidth]{fig_probes.png}\caption{}\end{subfigure}\hfill
\begin{subfigure}{0.49\textwidth}\includegraphics[width=\linewidth]{fig_umap_instance.png}\caption{}\end{subfigure}
\fnote{(a) Proxy-A-distance and event/domain linear-probe accuracy across the DCASE-T7 heads: the
sensitive head raises domain separability while the invariant head lowers it, and both
keep event-probe accuracy high. (b) UMAP of the instance head on sound events unseen
...
\begin{table}[h]
\centering
\caption{Contrastive supervision alone closes the gap; DANN, CORAL, and IRM add nothing (DCASE-T7).}
\label{tab:ablation}
\renewcommand{\arraystretch}{1.25}\small
\begin{tabular}{lcc}
\toprule
...
\begin{table}[h]
\centering
\caption{Held-out sound events collapse below frozen under class supervision (DCASE-T7 / UCS).}
\label{tab:closed}
\renewcommand{\arraystretch}{1.25}\small
\begin{tabular}{lcc}
\toprule
...
\begin{table}[h]
\centering
\caption{The dissociation at finer granularity (65 vs 34 sound events, CLAP, unseen events).}
\label{tab:fine}
\renewcommand{\arraystretch}{1.25}\small
\begin{tabular}{lcccc}
\toprule
...
\begin{table}[h]
\centering
\caption{Instance retrieval is deep and direction-symmetric (UCS, unseen events, full gallery).}
\label{tab:operating}
\renewcommand{\arraystretch}{1.2}\small
\begin{tabular}{llccc}
\toprule
...
\begin{table}[h]
\centering
\caption{The instance mapping is cheap to learn (instance head, subsampled training pairs).}
\label{tab:dataeff}
\renewcommand{\arraystretch}{1.2}\small
\begin{tabular}{lccccccc}
\toprule
...
\begin{table}[h]
\centering
\caption{The sensitive axis is a per-generator detector, not a graded fidelity meter.}
\label{tab:sensitive}
\renewcommand{\arraystretch}{1.2}\small
\begin{tabular}{lcc}
\toprule
...
UCS train split). The axis separates real audio from the training generator essentially perfectly
(AUC $1.000$) at every operating point, but two limits show it is a \emph{detector}, not a fidelity
...
almost real (AUC $0.747$ against real; $0.007$ against Stable-Audio---i.e.\ scored realer than the
training generator's own twins). Realness detection is thus per-generator, mirroring the instance
head's boundary.}
\end{table}

\begin{table}[h]
\centering
\caption{Off-the-shelf dedup methods cannot match twins to their source (UCS, unseen events).}
\label{tab:baselines}
\renewcommand{\arraystretch}{1.2}\small
\begin{tabular}{lccc}
\toprule
